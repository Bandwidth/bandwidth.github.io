...
| redirectFallbackUrl | (optional) A fallback url which, if provided, will be used to retry the answer callback delivery in case `redirectUrl` fails to respond | No |
| redirectFallbackMethod | (optional) The HTTP method to use to deliver the redirect callback to `redirectFallbackUrl` | No |
