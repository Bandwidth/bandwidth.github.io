...
<code class="post">POST</code>`https://voice.bandwidth.com/accounts/{accountId}/calls/{callId}`
...
```bash
curl -u username:password -v -X POST https://voice.bandwidth.com/accounts/{accountId}/calls/{callId}\
...
```bash
curl -v -X POST https://voice.bandwidth.com/accounts/{accountId}/calls/{callId}\
	-u {token}:{secret} \
	-H "Content-type: application/json" \
	-d \
	'
